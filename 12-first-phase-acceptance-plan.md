...
| 切片 | 验收方式 | 是否必须 |
| --- | --- | --- |
| 3.1 冻结 v0 契约样例 | 对照 `08-contract-examples.md` 检查对象样例 | 是 |
| 3.2 定义第一阶段演示闭环 | 人工确认演示场景和非目标 | 是 |
| 3.3 供端能力目录与默认调用策略 | 查询能力、额度和策略样例 | 是 |
| 3.4 生产任务最小运行骨架 | 创建任务并查看状态流转 | 是 |
| 3.5 工具节点调用闭环 | 触发一次工具调用和成本记录 | 是 |
| 3.6 质量门禁闭环 | 覆盖 pass、manual_review、rerun、fail | 是 |
| 3.7 人工干预暂停与恢复 | 触发人工审核并恢复 | 是 |
| 3.8 ProductPackage 生成与版本规则 | 生成 v1 和优化版本 | 是 |
| 3.9 深度调研报告生产线 MVP | 跑通报告生产和上架 | 是 |
| 3.10 视频生成生产线 MVP | 跑通视频商品包闭环 | 可简化，不阻塞报告线主闭环 |
| 3.11 销端接入商品包并展示 | 展示列表和详情 | 是 |
| 3.12 预览、下载与基础销售记录 | 记录购买或下载 | 是 |
| 3.13 评分评论与 MarketFeedback 回流 | 生成并回传反馈 | 是 |
| 3.14 反馈驱动优化任务 | 经运营确认从反馈创建优化任务 | 是 |
| 3.15 端到端追踪与运营查询 | 查询完整链路 | 是，不要求完整仪表盘 |
